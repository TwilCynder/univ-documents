...
class Buffer:
    ...
    def __init__(self, nb_cases, nb_types):
        ...
        self.prod_wait_counter = Array('i', [0] * nb_types)
    ...
    def produce(self, msg_val, msg_type, msg_source):
      with self.lock:    
        self.prod_wait_counter[msg_type] += 1
        while self.plein() or self.last_type.value   == msg_type:
        ...
        if self.nb_vides.value > 0 and self.prod_wait_counter[1-msg_type] > 0:
          self.cond_prod[1-msg_type].notify()
        else:
    ...
    def consume(self, id_cons):
      with self.lock:
        ...
        if self.prod_wait_counter[1-self.last_type.value] > 0:
          self.cond_prod[1-self.last_type.value].notify()
        else:
          self.cond_cons.notify()
        return result
