...
if rank == 0:
  ...
  pixels = split(pixels, nbProcesses)
...
if rank % 2 == 0: #ifing outside of the loop even if it means duplicating the loop code, to avoid checking at every iteration (performance)
  for i in range(0,len(pixels)):
...
pixelsArray = comm.gather(pixels, root = 0)

if rank == 0:
  end_time = time.time()

  pixels, n, t = unsplit(pixelsArray)

  # save the image
  saveImage(pixels, nblines, nbcolumns)
  print("Stretching done in", start_time - end_time, "s")
