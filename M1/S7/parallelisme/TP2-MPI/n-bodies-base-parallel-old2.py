...
for t in range(0, NBSTEPS):
	...
	for i in range(rank, nbbodies, nbProcesses):
		forceLocal = (0, 0)
		for j in range(rank, nbbodies, nbProcesses):
			forceLocal = addForces(forceLocal, interaction(data[i], data[j]))
		force.append(forceLocal)

	tmp = comm.gather(force, root = 0) #tmp est une liste de liste de vecteurs 
	
	if rank == 0:
		force = [[0,0] for _ in range(nbbodies)]
		for k in range(0, nbProcesses):
			for i in range(0, len(tmp[k])): #a priori tmp[k], la liste renvoyée par le process k, contient les forces k + i * nbprocesses avec i entier
				force[k + nbProcesses * i] = tmp[k][i]
		print(force)
