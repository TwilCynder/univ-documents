...
void premier_fils(int nb_tours){
  ...
  info.valeur_inc = 0;
    ...
    while (count < nb_tours && result > 0){ //result <= 0 signifique qu'il y a eu un problème
      info.valeur_inc++;
      ...
      count++;
      (result = read(pipe_lecture, &info, sizeof(info)));
    }
...
void fils(int mon_num){ //en considérant que le premier numéro est 0, et que ce numéro ne sera jamais passé (car le premier fils a une fonction séparée)
  ...
  for (int i = 0; i < nb_fils * 2; i++){
    if (i != num_pipe_ecriture && i != num_pipe_lecture){
      if (close(pipes[i])) error("Fermeture pipes premier fils", 2); //on ferme tout sauf l'entrée du premier pipe et la sortie du dernier pipe
    }
  }
...
int main(int argc, char const *argv[])
{
  ...
  if (argc < 3){
    fprintf(stderr, "Usage : %s nombre_de_fils nombre_de_tours\n", argv[0]);
    exit(1);
  }
