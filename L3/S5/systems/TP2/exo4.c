...
void retrait (TypeMessage *leMessage) {
  TypeMessage msgVide;
  strcpy(msgVide.info, "xxxxx");
  msgVide.type = -1;  
  strcpy(leMessage->info, leBuffer.tampon[leBuffer.iRetrait].info);
  strcpy(leBuffer.tampon[leBuffer.iRetrait].info, msgVide.info);
...
int main(int argc, char *argv[]) {
  ...
  sem_init(&sem_prod, 0, nbCases);
  sem_init(&sem_conso, 0, 0);
...
#ifdef TRACE_CREAT
    printf("Creation thread de rang %d -> Prod %d(%lu)/%d\n", i, rangThds[i], idThdProd[i], nbProd);
#endif
  }
  for (i = 0; i <  nbConso; i++) {
    rangThds[i+nbProd] = i;
    if ((etat = pthread_create(&idThdConso[i], NULL, consommateur, &rangThds[i+nbProd])) != 0)
      thdErreur(etat, "Creation consommateur", NULL);
#ifdef TRACE_CREAT
    printf("Creation thread de rang %d -> Conso %d(%lu)/%d\n", i+nbProd, rangThds[i+nbProd], idThdConso[i], nbConso);
#endif
  }
