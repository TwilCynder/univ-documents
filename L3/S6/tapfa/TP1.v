...
Fixpoint pair n := match n with
| O => true
| S n_ => negb (pair n_)
end.

Eval compute in pair 6.
Eval compute in pair 7.
...
Fixpoint inf n m :=
  match n,m with
  | O, _ => true (*O est toujours plus petit ou égal qu'un autre nat*)
  | S n1, O => false (*Le sucesseur d'un nat (donc au moins 1) est toujours plus grand que 0*)
  | S n1, S m1 => inf n1 m1 (*Si deux nombres ont une relation inférieur/supérieur, leurs prédecesseurs auront la même*)
  end.
Check inf.
...
Definition ex4 : (A -> B) -> (B -> C) -> A -> C := fun f0 f1 a => f1 (f0 a).

Definition ex5 : (A -> B) -> (A -> B -> C) -> A -> C := fun f0 f1 a => f1 a (f0 a).
