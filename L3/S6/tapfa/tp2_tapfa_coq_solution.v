...
Proof.
(* c'est faux, aucun espoir de réussir à le prouver *)
Abort.
...
Definition app (l l' : L.list_nat) : L.list_nat :=
  L.list_nat_it L.list_nat l' (fun x l res => L.cons x res) l.

Lemma app_length : forall l l', L.length (app l l') = L.length l + L.length l'.
Proof.
intros l l'.
apply (L.list_nat_ind
         (fun l => L.length (app l l') = L.length l + L.length l')).
- unfold app.
  rewrite L.list_nat_it_nil.
  rewrite L.length_nil.
  reflexivity.
- unfold app.
  intros a l'' IHl''.
  rewrite L.list_nat_it_cons.
...
Definition cons (x : nat) l := x :: l.

Fixpoint list_nat_it T (f_nil : T) f_cons (l : list_nat) :=
  match l with
  | [] => f_nil
  | x :: l => f_cons x l (list_nat_it T f_nil f_cons l)
  end.

Definition length := list_nat_it _ 0 (fun _ _ n => 1 + n).

Definition list_nat_ind := @list_ind nat.

Lemma list_nat_it_nil T f_nil f_cons : list_nat_it T f_nil f_cons nil = f_nil.
Proof. reflexivity. Qed.

Lemma list_nat_it_cons T f_nil f_cons x l :
  list_nat_it T f_nil f_cons (cons x l)
  = f_cons x l (list_nat_it T f_nil f_cons l).
Proof. reflexivity. Qed.
...
Lemma length_cons x l : length (cons x l) = 1 + length l.
Proof. reflexivity. Qed.
