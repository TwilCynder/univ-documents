...
static long ioctlchar(struct file *filp, unsigned int cmd, unsigned long arg)
{
	void __user *argp = (void __user *)arg;
	size_t len;

	switch (cmd) {
	case BUFFER_IOCTL_STRLEN:
		len = strlen(name);
...
MODULE_DESCRIPTION("Simple char device using udev and ioctl(2)");
